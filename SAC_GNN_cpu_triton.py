...
from graph_generator import create_whole_ego_bigraph_tensor,create_whole_ego_bigrapher_multi
#import traceback
...
from utils_gnn import save_model,  check_param_device
#import pygame
...
def parse_args():
    ...
    parser.add_argument("--learning-starts", type=int, default= 10000,
        help="timestep to start learning")#5e3
    ...
    parser.add_argument("--target-network-frequency", type=int, default=5, # Denis Yarats' implementation delays this by 2.
...
class SoftGATv2Network(nn.Module):
    ...
    def __init__(self,env,graph_data,device,num_heads=8,dropout=0.3):
        ...
        self.conv1 = GATv2Conv(in_channels= input_dim, out_channels= num_heads*input_dim,heads=num_heads,dropout=self.dropout,edge_dim=edge_dim,add_self_loops= False)
        
        self.conv2 = GATv2Conv(in_channels= -1 ,out_channels=2*num_heads*input_dim,heads=num_heads,dropout=self.dropout,edge_dim=edge_dim,add_self_loops=False)
        
        self.conv3 = GATv2Conv(in_channels= -1,out_channels= 4*num_heads*input_dim,heads=1,dropout=self.dropout,edge_dim=edge_dim,add_self_loops=False)
        
        self.dense = nn.Sequential(nn.Linear(4*num_heads*input_dim,output_dim_row),nn.Dropout(dropout),
    ...
    def forward(self,graph_data):
        ...
        # handle the Batch of graphs inputs  while training
        if isinstance(graph_data,Batch):
            new_ego_node_embeddings = torch.zeros((graph_data.num_graphs,self.output_dim_column),device=self.device) # Shape: (batch_size, embedding_size)
            for graph_index in torch.unique(batch):
                graph_mask = (batch == graph_index)
                new_ego_embed = x[graph_mask][0]
                new_ego_node_embeddings[graph_index] = new_ego_embed
        # for a single graph input while training
        else:
            new_ego_node_embeddings = x[0]
    
        new_ego_node_embeddings = new_ego_node_embeddings
        #print(x.device)
...
if __name__ == "__main__":
    ...
    vehicle_node_attribute_matrix = obs
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad(), torch.set_grad_enabled(False):
                ...
                #transfer to cpu for multiprocessing
                next_observations_cpu = data.next_observations.cpu()
                observations_cpu = data.observations.cpu()
                
                with profiler.profile(activities=[ProfilerActivity.CPU],profile_memory=True,record_shapes=True) as prof:
                # create the graphs in the cpus before tranfering to the cuda device
                   # with torch.no_grad(), torch.set_grad_enabled(False), torch.cuda.device('cpu'):
                    #mp.set_start_method('spawn')
                    pool = mp.Pool(processes=ncpus)
                    
                    #convert the next obsevations from replay buffer to graphs inputs 
                    next_results = [pool.apply_async(create_whole_ego_bigrapher_multi,args=(idx,next_observations_cpu[idx]))for idx in range(args.batch_size)]
                    next_ego_graph_batch = [next_result.get() for next_result in next_results ]
                    
                    
                    #convert the observations from replay buffer to graph inputs
                    results =  [pool.apply_async(create_whole_ego_bigrapher_multi,args=(idx,observations_cpu[idx]))for idx in range(args.batch_size)]
                    ego_graph_batch = [result.get() for result in results ]
                    
                    #convert to Batch of Data graphs - A data object describing a batch of graphs as one big (disconnected) graph
                    next_ego_graph_batch_cuda = Batch.from_data_list(next_ego_graph_batch).cuda()
                    #convert to Batch of Data graphs -A data object describing a batch of graphs as one big (disconnected) graph
                    ego_graph_batch_cuda = Batch.from_data_list(ego_graph_batch).cuda()
            ...
            with torch.no_grad():
                ...
                next_state_actions, next_state_log_pi, _ = actor.get_action(next_ego_graph_batch_cuda)
                qf1_next_target = qf1_target(next_ego_graph_batch_cuda, next_state_actions)
                qf2_next_target = qf2_target(next_ego_graph_batch_cuda, next_state_actions)
            ...
            qf1_a_values = qf1( ego_graph_batch_cuda, data.actions).view(-1)
            qf2_a_values = qf2( ego_graph_batch_cuda, data.actions).view(-1)
            ...
            if global_step % args.policy_frequency == 0:  # TD 3 Delayed update support
                for _ in range(
                    ...
                    pi, log_pi, _ = actor.get_action(ego_graph_batch_cuda)
                    qf1_pi = qf1(ego_graph_batch_cuda, pi)
                    qf2_pi = qf2(ego_graph_batch_cuda, pi)
        ...
        #    
                    ...
                    if args.autotune:
                        with torch.no_grad():
                            _, log_pi, _ = actor.get_action(ego_graph_batch_cuda)
