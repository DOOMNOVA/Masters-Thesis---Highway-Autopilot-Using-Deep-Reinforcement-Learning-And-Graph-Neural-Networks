...
#################Combined Vehicle graph generation code######################
...
def create_whole_ego_bigraph_tensor(node_attribute_matrix):
   ...
   # num_node_features = node_attribute_matrix.shape
    max_obs_vehicles = 12 # no of nodes
...
def create_whole_ego_bigraph_tensor_dynamic(node_attribute_matrix):
    ...
    # Filter known vehicles, used as a vehicle identifier in creating the edge indexes.
    known_vehicle_indices = torch.nonzero(presence).squeeze()
    
    # Extract node features for known vehicles.
    
    vehicle_node_features = torch.as_tensor(node_attribute_matrix[known_vehicle_indices, :],dtype=torch.float).clone().detach()
...
def create_egocentric_unidirec_tensor_dynamic(node_attribute_matrix):
...
def create_whole_ego_unigraph_tensor(node_attribute_matrix):
    ...
    num_vehicles  = vehicle_node_features.shape[0]
    # vehicle_node_features = torch.as_tensor(node_attribute_matrix, dtype=torch.float).to(device)
    # num_vehicles = vehicle_node_features.shape[0]
    ego_vehicle_idx = 0
    vehicle_indices = torch.arange(num_vehicles)

    src_indices, tgt_indices = torch.meshgrid(vehicle_indices, vehicle_indices)
